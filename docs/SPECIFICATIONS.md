...
Para ejecutar con Node.js/Playwright:

1. Instala dependencias: `npm install` (o `yarn`/`pnpm` según tu flujo).
2. Instala los navegadores de Playwright: `npx playwright install --with-deps`.
3. Ejecuta las specs: `npx playwright test`.
4. Ejecuta una spec concreta: `npx playwright test src/tests/hero-component-test.spec.ts`.

2) Usando Bun (solo para dev/build, Playwright puede requerir Node.js para test runner)

Si usas Bun para desarrollo:

1. Instala dependencias: `bun install`.
2. Inicia la app: `bun dev`.
3. En otra terminal ejecuta Playwright con Node.js: `npx playwright test`.
